...
Фронтенд приложение находится в директории `frontend/` и построен на React с TypeScript.
...
**Важно:** Убедитесь, что бэкенд запущен на http://localhost:8000 перед запуском фронтенда.

### Подробная документация фронтенда

См. [README фронтенда](./frontend/README.md) для детальной информации о структуре проекта, возможностях и разработке.
